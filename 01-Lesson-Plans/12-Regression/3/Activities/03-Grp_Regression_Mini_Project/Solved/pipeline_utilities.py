...
def preprocess_prod_data(prod_df, column_name):
    """
    ...
    """
    df = prod_df.dropna()
    X = pd.get_dummies(df.drop(columns=column_name))
...
def prod_model_generator(prod_df, model_name):
    """
    ...
    steps = [("Scale", StandardScaler(with_mean=False)), 
             ("Linear Regression", model_name())] 

    # Create a pipeline object
    pipeline = Pipeline(steps)

    # Apply the preprocess_rent_data step
    X_train, X_test, y_train, y_test = preprocess_prod_data(prod_df, column_name = 'actual_productivity')
    ...
    print(f'Score: {pipeline.score(y_test, y_pred)}')
    if r2_adj_value < 0.4:
